...
| Snapshot wire-format version — bump it in lockstep with any `SnapshotBody` or encoder change; `decode` rejects anything higher and the caller falls back to full replay (postcard since `4`, bincode through `3` — #207) | `outl_core::snapshot::SCHEMA_VERSION` | `crates/outl-core/src/snapshot.rs` |
