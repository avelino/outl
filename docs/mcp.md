...
The opener tries the config actor first and, if taken, mints an `ActorId::new()` ephemeral and locks that one instead — so the second `outl mcp serve` of the day usually just writes to a fresh `ops-<ephemeral>.jsonl` without telling you.
You only see a hard error when both the config actor AND the ephemeral path can't be acquired (e.g. a stale `.lock-<actor>` left by a killed process, or a filesystem that doesn't support `flock`).
