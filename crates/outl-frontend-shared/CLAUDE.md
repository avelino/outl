...
|---|---|---|
| `<MarkdownInline />` (refs/tags fire `onRefClick`/`onTagClick`; external `[label](url)` links fire the optional `onLinkClick(href)` — when wired, the link is a keyboard-operable button (`role`/`tabindex`/Enter+Space); when omitted it's a plain inert `<span>`, no fake button. `((blk-X))` block-refs resolve their `text` from the `embeds` map when present — Roam-style inline reference, TODO/DONE marker prefixed — and fall back to the neutral handle chip when orphan; `!((blk-X))` embeds still render the single-line `↳ text`; `![alt](href)` **image** tokens render an inline `<img>` for image extensions — a local `assets/…` asset resolved to a `data:` URL via `readAssetDataUrl`, a remote `http(s)` src used directly — and a clickable **file chip** for pdf / other kinds that fires `onLinkClick(href)` so the client opens it in the OS viewer, issue #203) | `@outl/shared/markdown` | output of `outl_md::tokenize_owned` |
...
| `isImagePath(href)` → `boolean` (extension classifier — strips `?`/`#`, matches the last `.`-segment; drives whether `<MarkdownInline />`'s `image` token renders an `<img>` or a click-to-open file chip) + `assetFileName(href)` (last path segment, for the chip label) | `@outl/shared/links` | `isImagePath` mirrors `IMAGE_EXTENSIONS` in `crates/outl-md/src/wikilink.rs` (`is_image_target`) — keep the extension list in sync |
