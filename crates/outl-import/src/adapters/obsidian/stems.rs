...
/// Walk the vault and compute the base slug for every `.md` file.
pub(super) fn discover(src: &Path) -> Result<Vec<DiscoveredFile>, ImportError> {
...
/// resolution order as the converter: frontmatter `title` → leading
/// H1 → filename stem.
