...
| `journal`   | `render_page_md`, `apply_page_md`, `apply_page_md_with_sidecar`, `apply_page_md_with_sidecar_if_absent` (lazy: projects only when `.md` is absent — use on read paths to avoid sidecar churn), `apply_all_pages_md`, `mutate_page_md`, `journals_dir`, `pages_dir`, `page_md_path`, `write_md_atomic` |
