...
| `outline`   | `OutlineNode` DTO + `project_outline` — UI-friendly tree projection. Also owns `flatten_subtree_paths` and `flat_index_for_block` (the DFS index used by `exec::run_code_block`). |
